    ...
    switch($action) {
      case 'install':
        try {
          ...
          $this->db->ExecuteQuery(self::SQL('insert content'), array('home', 'page', 'Home page', "This is a demo page, this could be your personal home-page.\n\nCowboy is based on <a href='https://github.com/mosbth/lydia.git'>Lydia</a>. Lydia is a PHP-based MVC-inspired Content management Framework, watch the making of Lydia at: http://dbwebb.se/lydia/tutorial.", 'htmlpurify', $this->user['id']));
          $this->db->ExecuteQuery(self::SQL('insert content'), array('about', 'page', 'About page', "This is a demo page, this could be your personal about-page.\n\nThe Cowboy Framework is based on <a href='https://github.com/mosbth/lydia.git'>Lydia</a>. Lydia is used as a tool to educate in MVC frameworks.", 'htmlpurify', $this->user['id']));
          $this->db->ExecuteQuery(self::SQL('insert content'), array('about', 'page', ' the Cowboy Framework', "The Cowboy Framework likes owls.\n<img src='../site/themes/mytheme/img/andy1.JPG'>\nThis particular owl enjoys wearing hats...", 'htmlpurify', $this->user['id']));
          $this->db->ExecuteQuery(self::SQL('insert content'), array('download', 'page', 'Download page', "This is a demo page, this could be your personal download-page.\n\nYou can download your own copy of Cowboy from https://github.com/AndyAkesson/CowboyFramework.", 'plain', $this->user['id']));
          ...
          $this->db->ExecuteQuery(self::SQL('insert content'), array('about', 'page', 'Welcome to the Cowboy Framework', "Detta ramverk är gjort med hjälp av en tutorial på webbplatsen dbwebb.se. Webbplatsen <a href='http://dbwebb.se'>dbwebb.se</a> är för de som vill lära sig - och hjälpa andra att lära sig - webbprogrammering och webbutveckling baserat på HTML, CSS, JavaScript, PHP och databer med SQL i Unix-miljö med fria programvaror. Här finns material att studera och likasinnade att diskutera med.<hr><h2>Kontakt</h2>Anders Åkesson, andy_akesson@me.com", 'htmlpurify', $this->user['id']));
